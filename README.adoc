...
Supernaut.Fx is designed for _modern_ JavaFX applications that use the latest version of JavaFX (currently 16) and are built for distribution using either the `jpackage` tool of JDK 16 or as native images using GraalVM JDK 11 (using native-image and Gluon Substrate)
...
** Applications built with JDK 16+ https://jdk.java.net/jpackage/[jpackage] (implemented)
